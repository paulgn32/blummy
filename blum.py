...
while True:
    ...
    print(Fore.YELLOW + Style.BRIGHT + f"1. [ IBHC ] Lounge")
    ...
    try:
        ...
        for index, query_id in enumerate(query_ids, start=1):
            ...
            while balance_info['playPasses'] > 0:
                ...
                time.sleep(10)
                claim_response = claim_game(token, game_response.get('gameId'), random.randint(230, 278))
                while True:
                    if claim_response is None:
                        ...
                        claim_response = claim_game(token, game_response['gameId'], random.randint(230, 278))
                        if claim_response is None:
                            print(f"\r{Fore.RED+Style.BRIGHT}[ Play Game ] : Gagal mengklaim game, mencoba lagi...", flush=True)
                ...
                if balance_info is None: # Refresh informasi saldo untuk mendapatkan tiket terbaru
                    ...
                    if balance_info['playPasses'] > 0:
                        print(f"\r{Fore.GREEN+Style.BRIGHT}[ Play Game ] : Tiket masih tersedia ({balance_info['playPasses']}), memainkan game lagi...", flush=True)
